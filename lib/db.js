...
function getPool() {
    ...
    // Enable SSL for any remote database (Aiven, Render, Supabase, etc.)
    // Use rejectUnauthorized:false for managed DBs with self-signed certs
    var isRemoteDb = connectionString.includes("@") && !connectionString.includes("localhost") && !connectionString.includes("127.0.0.1");
